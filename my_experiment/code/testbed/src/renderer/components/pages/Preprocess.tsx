...
export default function Preprocess() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-white">Pre-processing</h2>
        <p className="text-sm text-slate-400 mt-1">
          Convert video to YUV format and overlay QR codes for frame tracking
        </p>
      </div>

      <div className="bg-surface-secondary border border-slate-700 rounded-xl p-6 max-w-2xl">
        {/* File selection */}
        <div className="mb-5">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 bg-surface border border-slate-600 rounded-lg px-3 py-2 text-sm text-slate-200 placeholder-slate-500"
            />
            <button
              onClick={handleSelectFile}
              className="px-4 py-2 bg-surface-tertiary hover:bg-slate-600 text-slate-200 rounded-lg text-sm font-medium transition-colors flex items-center gap-2"
            >
              <FolderOpen size={16} />
              Browse
            </button>
        ...
        <div className="grid grid-cols-3 gap-4 mb-5">
          <div>
            ...
            <input
              ...
              min={64}
              max={7680}
              className="w-full bg-surface border border-slate-600 rounded-lg px-3 py-2 text-sm text-slate-200"
            />
          </div>
          <div>
        ...
        <div className="flex gap-3">
          <button
            ...
            className="px-5 py-2.5 bg-accent hover:bg-accent-hover disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg text-sm font-medium transition-colors flex items-center gap-2"
          >
            <Film size={16} />
        ...
        {/* Status */}
        {status && (
          <div className="mt-4 px-4 py-3 bg-surface rounded-lg border border-slate-600">
